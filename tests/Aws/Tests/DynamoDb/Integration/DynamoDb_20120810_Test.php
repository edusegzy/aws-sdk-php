...
class DynamoDb_20120810_Test extends \Aws\Tests\IntegrationTestCase
{
    ...
    public function testMarshaler()
    {
        ...
        $m = new Marshaler();
        $time = time();
        $json = '{"id":1500,"time":'.$time.',"people":[{"name":"Jim","alive":true},'
            . '{"name":"Bob","alive":false},{"name":"Amy","alive":true}]}';

        $client->putItem(array(
            'TableName' => 'errors',
            'Item' => $m->marshalDocument($json)
        ));
        ...
        $this->assertEquals(
            ...
            json_decode($m->unmarshalDocument($result['Item']), true)
        );
    }
